...
const formatAugustIso = (day: number, minuteOfDay: number) => {
  ...
  return `2026-08-${pad(day)}T${pad(hour)}:${pad(minute)}:00.000${LOS_ANGELES_AUGUST_2026.location.utcOffset}`
}
...
export const classifyTou = (dayOfWeek: number, minuteOfDay: number): TouPeriod => {
  ...
  if (
    (minuteOfDay >= 10 * 60 && minuteOfDay < 13 * 60) ||
...
export const buildAugustClock = (): ClockSlot[] => {
  ...
  for (let day = 1; day <= AUGUST_DAYS; day += 1) {
    ...
    for (
      ...
      minuteOfDay += LOS_ANGELES_AUGUST_2026.intervalMinutes
    ) {
      slots.push({
        index: slots.length,
